...
describe('Una linked list', function() {
  ...
  it('deberia poder tomar strings y funciones ambos como search inputs', function() {
    linkedList.add('one');
    linkedList.add('two');
    expect(linkedList.search(function(nodeValue) { // Lista head. Nodo value next. head.value = Lista[0].value = chot, .add head.next.value {valor next}
      return nodeValue === 'two'; //param() === "two"
    })).toBe('two');
  });
